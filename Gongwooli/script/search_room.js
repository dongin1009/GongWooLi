...
function request(fnr, index, length) {
  $.ajax({
      url:'https://ecguide.wku.ac.kr/Gongwooli/server/search_room.php?fnr=' + fnr,
      type: "GET",
...
function requestBySearchTerm(searchTerm) {
  ...
  if (error_flag) {
    $.ajax({
        url:'https://61.245.248.177/Gongwooli/server/search_fnr.php?search=' + searchTerm,
        type: "GET",
        success: function (data) {
          keywords = parseKeywordData(data);
          responseToFnr(keywords);
        }
    });
  }
}
...
function renderingRoomList(room) {
  ...
  innerhtml += '<div id="' + room.getRoomFnr() + '" class="search_list_item_wrap">';
  innerhtml +=      '<div class="room_number_wrap">';
  innerhtml +=          '<center><font>' + room.getRoomMajNo() + '</font></center>';
  innerhtml +=      '</div>';
  innerhtml +=      '<div class="room_title_wrap">';
  innerhtml +=          '<center><font>' + room.getRoomNm() + '</font></center>';
  innerhtml +=      '</div>';
  innerhtml +=      '<div class="room_category_wrap">';
  innerhtml +=          '<center><font>' + room.getUseNm() + '</font></center>';
  innerhtml +=      '</div>';
  innerhtml +=      '<div class="room_use_dept_wrap">';
  innerhtml +=          '<center><font>' + room.getUseDeptNm() + '</font></center>';
  innerhtml +=      '</div>';
  innerhtml +=      '<div class="room_user_wrap">';
...
function goToLocation(id) {
  ...
  location.href="./location.html?room_maj_no=" + room.getRoomMajNo()
...
function setClickListener() {
  $('.search_list_item_wrap').click(function(){
    var id = $(this).attr("id");
    goToLocation(id);
  });

  $('.search_list_item_wrap').bind("touchend",function(e){
    if(dragging)
      return;
  ...
  $('.search_list_item_wrap').bind("touchmove", function(){
      dragging = true;
  });
}
...
function responseToFnr(keywords) {
